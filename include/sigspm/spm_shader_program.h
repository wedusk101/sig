...
#include <string>
#include "spm_shader.h"

class SpmShaderProgram
{
public:
	SpmShaderProgram();
   ~SpmShaderProgram();
...
public:
	void Create( const SpmShaderSrc& csh );
	void Create( const SpmShaderSrc& vsh, const SpmShaderSrc& fsh );
	void Create( const SpmShaderSrc& vsh, const SpmShaderSrc& gsh, const SpmShaderSrc& fsh );
	void Delete();
	void Link();
	void Validate();
	void Reload();
...
private:
	GLuint ID;
	SpmShader vertexShader, geometryShader, fragmentShader, computeShader;
};
